...
select  * from ride where ride_giver_id='103';
select u.username from booking b join users u on b.passenger_id = u.id where b.ride_id = '204'; 
select * from ride where ride_giver_id = '103' or ride_id in (select ride_id from booking where passenger_id = '105');  
select * from ride where origin = 'Banglore' and destination = 'Mysore';  
select ride_id, count(passenger_id) as passenger_count from booking group by ride_id;  
select * from ride where departure ='2025-03-05 16:45:00'; 
select distinct u.username from users u join ride r on u.id = r.ride_giver_id where r.departure = '2025-03-05 16:45:00';  
select * from ride where origin = 'Kochi' and available_seats > 2;
...
select * from ride where origin = 'Tvm'and departure = '2025-03-03 12:00:00' and available_seats > 0;
select * from ride where origin = 'Banglore'or destination = 'Kozhikode';  
...
select * from ride where time(departure) between '10:30:00' and '15:00:00';
